...
| 自動ログイン | セッションが失効したときのフォールバック。ID/パスワードを Riot Client に自動入力 |
...
セッションが失効していて、かつ ID/パスワードが登録済みなら、自動入力ログインにフォールバックする。
...
- 自動入力ログインは Riot Client の画面構成に依存する。Riot 側の変更で壊れうるので、通常はセッション方式を使う
