...
| 自動ログイン | セッションが失効したときのフォールバック。ID/パスワードを入力するところまで。送信はしない |
...
**入力するところまでで、サインインは押さない。** Riot のログイン画面は hCaptcha で
保護されているうえ、「サインイン状態を維持」を有効にしてもらう必要があるため、
最後の一押しは利用者に任せている。

実機 (Riot Client v138.0.1) で確認した挙動:
...
- **ウィンドウを前面にしただけでは入力欄がフォーカスを持たない。** 必ずクリックしてから打つ
- ログインウィンドウはリサイズ不可の固定サイズなので、入力欄の位置は
  ウィンドウ相対の比率で足りる (`autologin.USERNAME_FIELD`)

> **「サインイン状態を維持」を必ず有効にすること。** これが無いと
